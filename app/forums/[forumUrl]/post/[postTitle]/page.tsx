...
import ForumQuePost from "./components/ForumQuePost";
import { Tiptap as TipTap } from "@/components/TipTap";
import ForumAnsPost from "./components/ForumAnsPost";
...
const ForumPostPage = ({ params: { postTitle } }: Props) => {
  ...
  const uploadImage = async(file: any) => {
    ...
    const storageRef = ref(storage, `forumPostAnswers/${file.name}`);
  ...
  async function createAnswerPost(data: Input) {
    ...
    const queRef = collection(db, "forumPosts");
    ...
    if (!snapshot.empty) {
      ...
      const docRef = await addDoc(collection(db, "forumPostAnswers"), {
  ...
  useEffect(() => {
    ...
    const queUnsub = onSnapshot(q, (snapshot) => {
      if (!snapshot.empty) {
        ...
        const ansRef = collection(db, "forumPostAnswers");
  ...
  return (
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-7 gap-y-4 md:gap-x-4 pb-6 font-dmsans mt-3">
      ...
      >
        ...
        <div className="">
          <ForumAnsPost answers={answers} postTitleWithSpaces={postTitleWithSpaces} postId={queObject.id} />
        </div>
      </div>
</div>
